...
class WCT {
	...
	public function init() {
		...
		if ( 1 == $options['enable_product_category_dependent_tabs'] ) {
			//add_action('admin_init', array( $this, 'taxonomy_for_objet_types' ));
			add_action( 'current_screen', array( $this, 'taxonomy_for_objet_types' ), 10, 1 ); //screen specific function only for product_tab editor admin pages, so we have to get the current screen type there 
		}
	...
	function display_woocommerce_custom_tabs( $tabs ) {
		//wsl_log(null, 'woocommerce-custom-tabs-pro.php display_woocommerce_custom_tabs 0: ' .  wsl_vartotext( '' ));
		...
		if ( !empty($tab_content) or 1 != $options['hide_empty_tabs'] ) {
			...
			if ($options['common_tabname'] != '') { //if common tabname is set, then this tab has to be displayed
				...
				'callback' 	=> array ($this, 'woocommerce_common_tab_content' )
				);
	...
	//call woocommerce_tab_content generator function with the 'common' tab code
	//owervrite lite tab code to 'common' (this is the stored tabe name)
	function woocommerce_common_tab_content($tab_code, $product_tabpage_post_ID ) {
		$this->woocommerce_tab_content('common_tab', $product_tabpage_post_ID );
	}

	//generate content of a specific product tab
	//function woocommerce_tab_content($tab_code , $product_tabpage_post_ID ) {
	function woocommerce_tab_content($tab_code, $product_tab ) {
		
		//wsl_log(null, 'woocommerce-custom-tabs-pro.php woocommerce_tab_content $tab_code: ' .  wsl_vartotext($tab_code));
		//wsl_log(null, 'woocommerce-custom-tabs-pro.php woocommerce_tab_content $product_tab: ' .  wsl_vartotext($product_tab));
		...
		if ($tab_code != 'common_tab') {
			...
			if ( isset($product_tab['wct_product_tabpage_post_ID'])) {
				$product_tabpage_post_ID = $product_tab['wct_product_tabpage_post_ID'];
			} else {
				return;
			}
			
			//wsl_log(null, 'woocommerce-custom-tabs-pro.php woocommerce_tab_content $product_tabpage_post_ID: ' .  wsl_vartotext($product_tabpage_post_ID));
	...
	function taxonomy_for_objet_types( $screen ) {
		
		//$screen = get_current_screen(); // we get it as a parameter from current_screen action
		
		//wsl_log(null, 'woocommerce-custom-tabs-pro.php taxonomy_for_objet_types $screen: ' .  wsl_vartotext($screen));
		
		//switch on "Product categories" metabox on tab type editor admin page for enable choosing a product category 
		//if it is set, then the counter on the top of the product category list page shows a wrong number, because it also counts the associated tab types, not only the products found for the category
		//register woocommerce product category to our product tabpage posttype
		
		if ( isset($screen) and $screen->id == 'product_tabpage' ) { //only on product tab editor page
